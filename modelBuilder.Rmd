...
In order to get things done faster, we pick out 5 % of entries from each type of data sources:
...
bigrams <- DocumentTermMatrix(myCorpus, control = list(tokenize = BigramTokenizer))
bigrams.noStop <- DocumentTermMatrix(myCorpus.noStop, control = list(tokenize = BigramTokenizer))
...
```{r createDFs, cache=TRUE, dependson=c("ngrams","corpusCleaning","corpusSubsetting", "readData")}

unigrams.dt <- 
    data.table(Term = unigrams$dimnames$Terms[unigrams$j],
               termFrequencyInDoc = unigrams$v)[, .(Frequency = sum(termFrequencyInDoc)), 
                                                by = Term][order(Frequency, decreasing = TRUE)]

bigrams.dt <- 
    data.table(Term = bigrams$dimnames$Terms[bigrams$j],
               termFrequencyInDoc = bigrams$v)[, .(Frequency = sum(termFrequencyInDoc)), 
                                                by = Term][order(Frequency, decreasing = TRUE)]
bigrams.noStop.dt <- 
    data.table(Term = bigrams.noStop$dimnames$Terms[bigrams.noStop$j],
               termFrequencyInDoc = bigrams.noStop$v)[, .(Frequency = sum(termFrequencyInDoc)), 
                                                      by = Term][order(Frequency, decreasing = TRUE)]

trigrams.dt <- 
    data.table(Term = trigrams$dimnames$Terms[trigrams$j],
               termFrequencyInDoc = trigrams$v)[, .(Frequency = sum(termFrequencyInDoc)), 
                                                by = Term][order(Frequency, decreasing = TRUE)]

quadgrams.dt <- 
    data.table(Term = quadgrams$dimnames$Terms[quadgrams$j],
               termFrequencyInDoc = quadgrams$v)[, .(Frequency = sum(termFrequencyInDoc)), 
                                                by = Term][order(Frequency, decreasing = TRUE)]

```


Ok, let's inspect the 1-grams and see how many words are needed to 

```{r}
Proportion <- cumsum(unigrams.dt$Frequency)/sum(unigrams.dt$Frequency)

plot(Proportion)

which.max(Proportion > 0.95)/length(Proportion)
```
...
ggplot(data=head(unigrams.dt, n = 25), 
...
ggplot(data=head(bigrams.dt, n = 25), 
       aes(x = reorder(Term, Frequency), y=Frequency)) + 
    geom_bar(stat = "identity") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5, size = 10),  
          axis.text.y = element_text(size = 12), 
...
ggplot(data=head(bigrams.noStop.dt, n = 25), 
       aes(x = reorder(Term, Frequency), y=Frequency)) + 
    geom_bar(stat = "identity") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5, size = 10),  
          axis.text.y = element_text(size = 12), 
...
ggplot(data=head(trigrams.dt, n = 25), 
       aes(x = reorder(Term, Frequency), y=Frequency)) + 
    geom_bar(stat = "identity") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5, size = 10),  
          axis.text.y = element_text(size = 12), 
...
ggplot(data=head(quadgrams.dt, n = 30), 
       aes(x = reorder(Term, Frequency), y=Frequency)) + 
    geom_bar(stat = "identity") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5, size = 10),  
          axis.text.y = element_text(size = 12), 
...
potentialAnswers <- 
    list(c("soda","beer","pretzels","cheese"), # B: soda X, pretzels X, 
         ...
         c("asleep","insensitive","callous","insane")) # callous X, insane
...
# Assumes 
getLastNWords <- function(text2extractWords, lastN){
    # check if class is "character"
    # check if only one element

    # Remove leading and trailing whitespace
    text2extractWords <- gsub(" +", " ", text2extractWords)
    # Remove double or more spaces:
    text2extractWords <- trimws(text2extractWords, "both")
    wordSample <- unlist(strsplit(text2extractWords, " "))
    wordSample[(length(wordSample)-lastN+1):length(wordSample)]
}

text2check <- getLastNWords(myText[1],4)

quadgrams.dt[grep(text2check,Term)]
trigrams.dt[grep(text2check,Term)]
